...
当前状态（2026-09-03，S/F/G 轮之后）：

- **计算内核**（`lib/core/`）：表达式解析、场采样、RK4 与自适应 Dormand–Prince 积分、雅可比、平衡点分类（带诚实的 caveat）、数值求平衡点；一阶方程以微分形式 `M dx + N dy = 0` 为底层（`dy/dx = g` 是特例），常数解沿整条直线检验、方向场奇点、七种标准形式的数值识别（只说「在数值上表现得像」，永远带 caveat）、恰当方程的势函数与隐式解等值线。
- **MCP 工具层**：`analyze_system`、`trace_trajectory`、`sample_field`、`analyze_first_order`（`expr` 或 `M`+`N`），加链路探针 `ping`。每个工具有 `locale` 参数（`zh` / `en`，默认 `en`），摘要文字全部来自双语文案表。
...
| `analyze_first_order(expr 或 M+N, xMin.., density, locale)` | 斜率场 / 方向场（微分形式画无向线段）、常数解与稳定性、方向场奇点、「在数值上表现得像」哪些标准形式（可分离、自治、对 y 线性、齐次、Bernoulli、恰当、积分因子）、恰当时的隐式解等值线 | 单个一阶方程：Logistic、牛顿冷却、可分离、线性、恰当方程、斜率场、「这题用什么方法」 |
| `ping(message)` | 原样返回 | 链路探针，判断是传输层挂了还是只有渲染挂了 |

`locale`：学生用中文提问传 `zh`，其他一律 `en`（默认）。摘要、caveat、类型证据都按这个语言生成；错误信息（参数越界、表达式解析失败）始终是英文，那是给模型看的。
...
诚实性规则：纯虚特征值只报 `center_or_weak_spiral` 并附 caveat（线性化分不清中心和弱螺旋）；行列式约等于零报 `non_hyperbolic` 并附 caveat（Hartman–Grobman 不适用）；方程类型只说「在数值上表现得像 X」并附 caveat（采样点上的恒等式检验不是证明），一个都不匹配时给正面说明（斜率场和数值解照样有效）。caveat 是给学生念的完整句子，中英各一份。

## 交互（网页外壳与 widget 相同）

- 视口等比：输入的范围放进画布时保持 x、y 像素比例相同，短的一边对称扩大，图下一行小字给出实际显示范围。
- 滚轮缩放（光标下的点不动，相对输入范围限制 1/50..50 倍），拖动平移，双击回到输入范围。
- 悬停预览经过该点的解曲线（`requestAnimationFrame` 节流，正逆各 ≤ 400 步），点击固定（正向蓝色、逆向橙色）。靠近方向场奇点时不画预览，改为提示「方向无定义」。
- 视图变化时场立即重采样；平衡点、常数解、奇点、类型、等值线在停止操作 250 毫秒后按可见范围重算。
...
- 下一步：Claude 实机验证 widget 交互；工具 description 按实际对话反复调（尤其 `locale` 是否被正确传入）；部署 Vercel。
