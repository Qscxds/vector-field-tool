    ...
    help: {
      ...
      controls: {
        ...
        form: [
          ...
          "讲课模式（页面右上角的按钮）：投屏讲课用。隐藏具体数值（特征值、迹和行列式、平衡点坐标、常数解的数值、偏差与阈值、范围行、查询的数值），放大文字；保留分类名、图上的标记、「与可分离方程一致」这类结论，以及短句形式的注意事项——「中心或弱螺旋」绝不会缩成「中心」，唯一性失效的「!」在任何模式下都在。每一行仍有 ⓘ，点开就是这一行的全部内容；切换不重新计算，退出后所有数字立刻回来。导出的 PNG 跟随当前模式（底部仍写明方程、参数取值和范围）。链接里是 lecture=1，嵌入页同样支持；Claude 读到的内容不受影响。",
          "网格密度：每个方向上箭头的个数。",
