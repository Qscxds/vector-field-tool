    ...
    help: {
      ...
      reading: {
        ...
        markers: [
          ...
          "分界线：从每个鞍点沿特征方向出发的四条粗黑曲线（稳定流形实线，逆时间积分；不稳定流形虚线，正时间积分）。它们把相平面分成命运不同的区域：初值在分界线这一侧还是那一侧，解的去向完全不同。它是数值曲线，从离鞍点极近（约一个像素）的一点沿线性化方向出发。单摆相邻两个鞍点之间，一个鞍点的不稳定流形和另一个的稳定流形是同一条曲线，两条线重合，图上只看得见一条；这种鞍点到鞍点的连接是结构不稳定的（方程稍加扰动，例如加一点阻尼，就会断开），工具只是把两条曲线各自画出来，不对「它们是否真的相连」做数值判定。",
        ],
    ...
    help: {
      ...
      reading: {
        ...
        markers: [
          ...
          "Separatrices: the four bold dark curves that leave each saddle along its eigen-directions (stable manifold solid, integrated backward in time; unstable manifold dashed, integrated forward). They divide the phase plane into regions with different fates: an initial point on one side of a separatrix and one on the other go to entirely different places. They are numerical curves, started about a pixel away from the saddle on the linearized direction. Between two neighbouring saddles of the pendulum, the unstable manifold of one and the stable manifold of the other are the same curve, so the two lines coincide and only one is seen; such a saddle-to-saddle connection is structurally unstable (the slightest perturbation, a little damping for instance, breaks it), and the tool only draws the two curves: it makes no numerical judgement on whether they really connect.",
        ],
