 ...
 * grows upward.
 *
 * Viewports built with fitViewport have the SAME pixel scale on both axes: a slope of 1 is drawn
 * at 45 degrees and the circular orbits of the harmonic oscillator are circles. The user's box is
 * treated as "show at least this much": the shorter direction is widened symmetrically.
 */
...
/**
 * Equal-scale viewport that shows at least `box`. Whichever direction would otherwise be drawn
 * with more pixels per unit is widened symmetrically until both scales agree.
 */
export function fitViewport(box: Box, width: number, height: number): Viewport {
...
export function resetViewport(originalBox: Box, width: number, height: number): Viewport {
  return fitViewport(originalBox, width, height);
}
