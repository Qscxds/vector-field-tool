...
describe("exactPotential", () => {
  it("2ty dt + (t² + y²) dy = 0: F = t²y + y³/3 from base (0,0), path-independent", () => {
    const pot = exactPotential({ kind: "differential", M: "2*t*y", N: "t^2 + y^2" }, box, { base: { x: 0, y: 0 } });
  ...
  it("flags a non-exact equation through path dependence: y dt - t dy = 0", () => {
    // Horizontal-then-vertical from (0,0) to (1,1): ∫₀¹ 0 ds + ∫₀¹ (-1) du = -1.
    // Vertical-then-horizontal: ∫₀¹ 0 du + ∫₀¹ 1 ds = +1. They disagree: not exact.
    const pot = exactPotential({ kind: "differential", M: "y", N: "-t" }, box, { base: { x: 0, y: 0 } });
    expect(pot.consistent).toBe(false);
    expect(pot.pathDeviation).toBeGreaterThan(0.1);
    expect(pot.F({ x: 1, y: 1 })).toBeCloseTo(-1, 12);
  });

  it("works for an exact equation with transcendental terms: (cos t · y) dt + (sin t + 2y) dy = 0", () => {
    // ∂M/∂y = cos t = ∂N/∂t. F = y sin t + y². From (0,0): F(π/2, 1) = 1 + 1 = 2.
    const pot = exactPotential({ kind: "differential", M: "cos(t)*y", N: "sin(t) + 2*y" }, box, { base: { x: 0, y: 0 }, panels: 128 });
    expect(pot.consistent).toBe(true);
    expect(pot.F({ x: Math.PI / 2, y: 1 })).toBeCloseTo(2, 8);
  });

  it("uses the box centre as the default base", () => {
    const pot = exactPotential({ kind: "differential", M: "t", N: "y" }, { x: { min: 0, max: 2 }, y: { min: 0, max: 4 } });
    expect(pot.base).toEqual({ x: 1, y: 2 });
    expect(pot.F(pot.base)).toBe(0);
    // F = (t² + y²)/2 - (1 + 4)/2: F(2, 4) = 10 - 2.5 = 7.5
...
describe("path-independence check is scale-free (review C6)", () => {
  ...
  it("the closed-but-not-exact form dθ fails the check whether or not it is multiplied by 1e-8", () => {
    // (t dy - y dt)/(t² + y²) is closed but has no potential on a box containing the origin: the two
    // integration paths differ by multiples of 2π wherever they wind differently around the origin.
    const plain = exactPotential({ kind: "differential", M: "-y/(t^2 + y^2)", N: "t/(t^2 + y^2)" }, box);
    const tiny = exactPotential({ kind: "differential", M: "-1e-8*y/(t^2 + y^2)", N: "1e-8*t/(t^2 + y^2)" }, box);
    const huge = exactPotential({ kind: "differential", M: "1e8*(-y)/(t^2 + y^2)", N: "1e8*t/(t^2 + y^2)" }, box);
  ...
  it("a genuinely exact equation passes at any scale", () => {
    for (const k of ["1", "1e-9", "1e9"]) {
      const r = exactPotential({ kind: "differential", M: `${k}*2*t*y`, N: `${k}*(t^2 + y^2)` }, box);
